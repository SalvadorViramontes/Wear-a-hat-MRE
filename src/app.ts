...
type HatDescriptor = {
	displayName: string;
	resourceName: string;
	scale: XYZ_Coordinates;
	rotation: XYZ_Coordinates;
	position: XYZ_Coordinates;
	menuPosition: XYZ_Coordinates;
	buttonSize: XYZ_Coordinates;
	buttonPosition: XYZ_Coordinates;
};
...
export default class WearAHat {
	...
	private menuHats: { [key: string]: MRE.Actor } = {};
	private menuLabels: { [key: string]: MRE.Actor } = {};
	...
	private showHatMenu() {
		...
		// Create menu button
		const transparentMaterial = this.assets.createMaterial('transparent', {
			color: MRE.Color4.FromHexString("000000ff"),
		...
		for (const hatId of Object.keys(HatDatabase)) {
			...
			let hatRecord = HatDatabase[hatId];
			let buttonMesh = this.assets.createBoxMesh('button', hatRecord.buttonSize.x, hatRecord.buttonSize.y, hatRecord.buttonSize.z);

			let hat: MRE.Actor;
			if(hatId !== 'none'){
				hat = MRE.Actor.CreateFromPrefab(this.context, {
					...
					actor: {
						...
						transform: {
							local: {
								position: { 
									x: hatRecord.menuPosition.x,
									y: hatRecord.menuPosition.y + y,
									z: hatRecord.menuPosition.z
								},
				...
				hat = MRE.Actor.Create(this.context, {
					actor: {
						parentId: menu.id,
						name: 'None',
						text: {
			...
			const button = MRE.Actor.Create(this.context, {
				actor: {
					...
					appearance: {
						...
						materialId: transparentMaterial.id
					},
					collider: { geometry: { shape: MRE.ColliderType.Auto } },
					transform: {
						local: {
							position: {
								x: hatRecord.buttonPosition.x,
								y: hatRecord.buttonPosition.y + y,
								z: hatRecord.buttonPosition.z
							}
						}
					}
				}
			...
			let label = MRE.Actor.Create(this.context, {
				actor: {
					parentId: menu.id,
					name: 'label',
					text: {
		...
		MRE.Actor.Create(this.context, {
			actor: {
				...
				name: 'label',
				text: {
					contents: ''.padStart(8, ' ') + "Wear some cool hats",
					height: 0.8,
